...
{
    sDeleteRChar(str, chr);
    sDeleteLChar(str, chr);
}
...
void ToUpper(char *str)
{
    ...
    for (unsigned int ii=0;ii < istrlen; ii++)
    {
...
{
    for (unsigned int i = 0; i < strlen(rmChar); ++i)
    {
        // you need include <algorithm> to use general algorithms like std::remove()
        str.erase(std::remove(str.begin(), str.end(), rmChar[i]), str.end());
    }
}
...
void cUpdateStr(char *str, const char *str1, const char *str2, bool bloop)
{
    if (str == nullptr) return;
    if (strlen(str) == 0) return;
...
bool UpdateStr(std::string& resource_str, const std::string& sub_str, const std::string& new_str, const bool bloop)
{
    ...
    // 如果bloop为true并且new_str中包函了sub_str的内容，直接返回，因为会进入死循环，最终导致内存溢出。
    if (bloop && std::string::npos != new_str.find(sub_str))
    {
        return false;
    }
    ...
    if (bloop)
    {
        while(std::regex_search(resource_str, e))
        {
            resource_str = std::regex_replace(resource_str, e, new_str);
        }
    }
    else
    {
...
bool MatchStr(const std::string& str, const std::string& rules)
{
    ...
    for (ii=0;ii<CmdStr.CmdCount();ii++)
    {
        ...
        for (jj=0;jj<CmdSubStr.CmdCount();jj++)
        {
            ...
            if (iPOS2 == std::string::npos) break;
...
bool CFile::Open(const char *filename, const char *openmode, bool bEnBuffer)
{
    ...
    if ( (m_fp=FOPEN(filename,openmode)) == nullptr ) return false;
...
bool CFile::OpenForRename(const char *filename, const char *openmode, bool bEnBuffer)
{
    ...
    if ( (m_fp=FOPEN(m_filenametmp,openmode)) == nullptr ) return false;
...
bool CFile::Fgets(char *buffer, size_t readsize, bool bdelcrt)
{
    if ( m_fp == nullptr ) return false;

    memset(buffer,0,readsize+1);  // 调用者必须保证buffer的空间足够，否则这里会内存溢出。

    if (fgets(buffer,readsize,m_fp) == nullptr) return false;
...
bool CFile::FFGETS(char *buffer, const size_t readsize, const char *endbz)
{
    if ( m_fp == nullptr ) return false;
...
{
	if (!CDir::isRegularFile(filename))
	{
		return false;
	}
	...
	{
		sDeleteRChar(tmp, '\r'); // Windows文件将会有'\r', 删除
		vecContent.push_back(tmp);
	}
	fin.close();
...
bool FGETS(const FILE *fp, char *buffer, const size_t readsize, const char *endbz)
{
    ...
    while (true)
    {
        ...
        if (fgets(strline,readsize,(FILE *)fp) == nullptr) break;
        ...
        if (endbz == nullptr) return true;
        else if (strstr(strline,endbz)!= nullptr) return true;
    }
...
void CCmdStr::SplitToCmd(const std::string& buffer, const char *sepstr, const bool bdelspace)
{
	...
	std::size_t ipos = 0;
	std::size_t lpos = 0;
	std::size_t len = strlen(sepstr);
	std::string tmp;
	while(std::string::npos != (ipos = buffer.find(sepstr, lpos)))
	{
		tmp = buffer.substr(lpos, ipos - lpos);
		if (bdelspace)
		{
			sDeleteLRChar(tmp, ' ');
		}
		m_vCmdStr.push_back(tmp);
		lpos = ipos + len;
	}
	tmp = buffer.substr(lpos);
	m_vCmdStr.push_back(tmp);
}

size_t CCmdStr::CmdCount()
{
    return m_vCmdStr.size();
}

bool CCmdStr::GetValue(const size_t inum, std::string& value, const size_t ilen)
{
    if (inum>=m_vCmdStr.size()) return false;

    if ( (m_vCmdStr[inum].length()<=ilen) || (ilen<=0) )
    {
        value = m_vCmdStr[inum];
    }
    else
    {
        value = m_vCmdStr[inum].substr(0, ilen);
    }
...
bool GetXMLBuffer(const char *xmlbuffer, const char *fieldname, char *value, const size_t ilen)
{
    ...
    size_t m_NameLen = strlen(fieldname);
    ...
    size_t  m_ValueLen = end - start - m_NameLen - 2 + 1 ;
...
void CLogFile::Close()
{
    if (m_tracefp != nullptr) { fclose(m_tracefp); m_tracefp=nullptr; }
...
bool CLogFile::Open(const char *filename, const char *openmode, bool bBackup, bool bEnBuffer)
{
    ...
    m_bEnBuffer=bEnBuffer;
    m_bBackup=bBackup;
    if (openmode==nullptr) strcpy(m_openmode,"a+");
    else strcpy(m_openmode,openmode);

    if ((m_tracefp=FOPEN(m_filename,m_openmode)) == nullptr) return false;
...
{
    if (m_tracefp == nullptr) return false;
    ...
    {
        fclose(m_tracefp); m_tracefp=nullptr;
...
// bisfilename：说明pathorfilename的类型，true-pathorfilename是文件名，否则是目录名，缺省值为false为目录。
...
bool MKDIR(const char *filename, bool bisfilename)
{
    ...
    size_t sizeFilename = strlen(filename) + 1;
    char strPathName[sizeFilename];
    char sTmp[sizeFilename];
    memset(sTmp, 0, sizeFilename);
    strcpy(sTmp, filename);
    if ('/' == filename[sizeFilename - 2])
    {
        sTmp[sizeFilename - 2] = '\0'; // 如果是'/'结尾那么删除它
    }

    for (size_t ii = 1; ii < strlen(sTmp); ii++)
    {
        if (sTmp[ii] != '/')
        {
            continue;
        ...
        strncpy(strPathName, sTmp, ii);

        if (0 == access(strPathName, F_OK))
        {
            continue;
        }

        if (0 != mkdir(strPathName, 0755))
        {
            return false;
        }
    }
    ...
    {
        if (0 != access(sTmp, F_OK)) // 如果为目录且不存在, 则创建最后一个目录
        {
            if (0 != mkdir(sTmp, 0755))
            {
                return false;
            }
        }
...
{
    if (MKDIR(filename, true) == false) return nullptr;
...
{
    char strtime[20],yyyy[5],mm[3],dd[3],hh[3],mi[3],ss[3];
...
{
    if ( (in_stime==nullptr) || (out_stime==nullptr) ) return false;
...
bool FileMTime(const char *filename, char *mtime, const char *fmt)
{
    ...
    if (fmt==nullptr) strcpy(strfmt,"yyyymmddhh24miss");
...
void CDir::initdata()
{
    ...
    m_FullFileName = "";
    m_FileSize=0;
    memset(m_ChangeTime,0,sizeof(m_ChangeTime));
...
// pathDepth, 限制递归深度, 若不指定(即pathDepth==-1)则默认不限制.
// 返回值：如果in_DirName参数指定的目录不存在，OpenDir方法会创建该目录，如果创建失败，返回false，还有，如果当前用户对in_DirName目录下的子目录没有读取权限也会返回false，其它正常情况下都会返回true。
bool CDir::OpenDir(const char *in_DirName, const char *in_MatchStr, const unsigned int in_MaxCount, const bool bAndChild, const bool bIgHidden, bool bSort, int pathDepth)
{
    m_pos=0;
    m_vFileName.clear();
    initdata();
    ...
    bool bRet = _OpenDir(in_DirName,in_MatchStr,in_MaxCount,bAndChild,bIgHidden,pathDepth);
...
bool CDir::_OpenDir(const char *in_DirName, const char *in_MatchStr, const unsigned int in_MaxCount, const bool bAndChild, const bool bIgHidden, int pathDepth)
{
    if (pathDepth == 0)
    {
        return true;
    ...
    if ( nullptr == (dir=opendir(in_DirName)) )
    {
        return false;
    }
    ...
    while ((st_fileinfo=readdir(dir)) != nullptr)
    {
        if (true == bIgHidden)
        {
            // 以"."打头的文件均不处理
        ...
        {
            // 忽略"." , ".."
            if ((0 == strcmp(st_fileinfo->d_name, ".")) || (0 == strcmp(st_fileinfo->d_name, "..")))
            {
                continue;
            }
        }
        ...
        if (S_ISDIR(st_filestat.st_mode))
        {
            if (bAndChild == true)
            {
                ...
                {
                    isSinglePath = false;
                    if (_OpenDir(strTempFileName,in_MatchStr,in_MaxCount,bAndChild,bIgHidden,--pathDepth) == false)
                    {
...
bool CDir::ReadDir()
{
    ...
    // 目录名
    m_DirName = m_vFileName[m_pos].substr(0,pos);

    // 文件名
    m_FileName = m_vFileName[m_pos].substr(pos+1, m_vFileName[m_pos].size()-pos-1);

    // 文件全名，包括路径
    m_FullFileName = m_vFileName[m_pos];

    struct stat st_filestat;

    stat(m_FullFileName.c_str(),&st_filestat);
    ...
    if (strcmp(m_DateFMT,"yyyy-mm-dd hh24:mi:ss") == 0)
    {
        ...
        snprintf(m_ChangeTime,20,"%04u-%02u-%02u %02u:%02u:%02u",\
                 nowtimer.tm_year+1900,nowtimer.tm_mon,nowtimer.tm_mday,\
                 nowtimer.tm_hour,nowtimer.tm_min,nowtimer.tm_sec);
...
bool CDir::rmAll(const char *in_DirName, const bool isRmFile)
{
    // 判断文件名是否存在
    if (0 != access(in_DirName, F_OK))
    {
        return true;
    }
    ...
    // 判断是否是目录
    struct stat st_filestat;
    stat(in_DirName, &st_filestat);
    if (!S_ISDIR(st_filestat.st_mode))
    {
        if (isRmFile)
        {
            if (0 != remove(in_DirName))
            {
                return false;
            }
        }
    ...
    while (nullptr != (st_fileinfo = readdir(dir)))
    {
        // 忽略"." , ".."
        if ((0 == strcmp(st_fileinfo->d_name, ".")) || (0 == strcmp(st_fileinfo->d_name, "..")))
        {
        ...
        strTempFileName = std::string(in_DirName) + std::string("/") + std::string(st_fileinfo->d_name);
        UpdateStr(strTempFileName, "//", "/");

        stat(strTempFileName.c_str(), &st_filestat);

        // 判断是否是目录
        if (S_ISDIR(st_filestat.st_mode))
        {
            if (false == CDir::rmAll(strTempFileName.c_str()))
            {
                closedir(dir); return false;
            }
        }
        ...
        {
            if (0 != remove(strTempFileName.c_str()))
            {
                closedir(dir); return false;
            }
        }
...
bool CDir::isExist(const char *in_DirName)
{
    if (0 == access(in_DirName, F_OK))
    {
        return true;
...
std::string CDir::getParentPath(const char *in_DirName)
{
    if (0 == strcmp(in_DirName, "/"))
    {
        return "";
    }

    std::string tmpString = CDir::getFullPath(in_DirName);
    UpdateStr(tmpString, "//", "/");
    if ('/' == tmpString.back())
    {
        tmpString.pop_back();
    ...
    return tmpString.substr(0, tmpString.rfind("/"));
}

bool CDir::isDirectory(const char *in_DirName)
{
    struct stat st_filestat;
    stat(in_DirName, &st_filestat);
    if (S_ISDIR(st_filestat.st_mode))
...
bool CDir::isRegularFile(const char *in_DirName)
{
    struct stat st_filestat;
    stat(in_DirName, &st_filestat);
    if (S_ISREG(st_filestat.st_mode))
    {
        return true;
    }
...
bool CDir::isEmptyDir(const char *in_DirName)
{
    DIR *dir;
    if (nullptr == (dir=opendir(in_DirName)))
    {
        // 目录不存在或者是非目录文件或者没有权限都返回false
        return false;
    }
    else
    {
...
bool RENAME(const char *srcfilename, const char *dstfilename, const int times)
{
    ...
    if (MKDIR(dstfilename, true) == false) return false;
...
bool CTcpClient::ConnectToServer(const char *ip, const unsigned short port)
{
...
bool CTcpClient::Write(const char *buffer, const size_t ibuflen)
{
    if (m_sockfd == -1) return false;
...
bool CTcpServer::Write(const char *buffer, const size_t ibuflen)
{
    if (m_connfd == -1) return false;
...
bool TcpRead(const int sockfd, char *buffer,int *ibuflen, const int itimeout)
{
    ...
    if (itimeout > 0)
    {
        ...
        int i;
        if ( (i = select(sockfd+1,&tmpfd,nullptr,nullptr,&timeout)) <= 0 ) return false;
    }
...
bool TcpWrite(const int sockfd, const char *buffer, const size_t ibuflen)
{
    ...
    if ( select(sockfd+1,nullptr,&tmpfd,nullptr,&timeout) <= 0 ) return false;
...
bool SendFile(int sockfd, struct st_fileinfo *stfileinfo, CLogFile *logfile)
{
    ...
    {
        if (logfile!=nullptr) logfile->Write("SendFile TcpWrite() failed.\n");
        return false;
    }
    ...
    if ( (fp=FOPEN(stfileinfo->filename,"rb")) == nullptr )
    {
        if (logfile!=nullptr) logfile->Write("SendFile FOPEN(%s) failed.\n",stfileinfo->filename);
        return false;
    }
    ...
    while (true)
    {
        ...
        if (bytes > 0)
        {
            ...
            {
                if (logfile!=nullptr) logfile->Write("SendFile Writen() failed.\n");
        ...
        if (total_bytes == stfileinfo->filesize) break;
    }
    ...
    {
        if (logfile!=nullptr) logfile->Write("SendFile TcpRead() failed.\n");
        return false;
    }
...
bool RecvFile(int sockfd, struct st_fileinfo *stfileinfo, CLogFile *logfile)
{
    ...
    if ( (fp=FOPEN(strfilenametmp,"wb")) ==nullptr)     // FOPEN可创建目录
    {
        if (logfile!=nullptr) logfile->Write("RecvFile FOPEN %s failed.\n",strfilenametmp);
        return false;
    }
    ...
    while (true)
    {
        ...
        {
            if (logfile!=nullptr) logfile->Write("RecvFile Readn() failed.\n");
            fclose(fp); fp=nullptr; return false;
        }
    ...
    if (TcpWrite(sockfd,strSendBuffer)==false)
    {
        if (logfile!=nullptr) logfile->Write("RecvFile TcpWrite() failed.\n");
        return false;
    }
...
bool COPY(const char *srcfilename, const char *dstfilename)
{
    ...
    long iFileSize=FileSize(srcfilename);
...
CRand::CRand()
{
    ...
    srand(begin.tv_usec); // 微妙计时器播种
}

bool CRand::checkexit(const size_t aryyval, const size_t aryysize)
{
...
void CRand::Rand(const size_t minvalue, const size_t maxvalue, const size_t nog, bool brep)
{
    size_t len = maxvalue-minvalue;
    size_t ii=0, jtmp=0;
    size_t itmp = 0;
    m_val.clear();
...
void itoa(int n, char *s)
{
    ...
    if (sign < 0) n = -n; // 考虑负数情况

    do
    {
        s[ii++] = n % 10 + '0';  // 注意, 此处必须 + '0'
    } while ((n /= 10) > 0) ;
...
{
    /*1.定义一个二维数组, len1行len2列.先找出相等的字符
     *2.遍历str1和str2, 若相等, 二维数组对应的行列值置为1, 不相等置为0, 连续相等的+1
     *3.最大长度即为该数组元素的最大值
     *4.再根据数组最大值的下标找到出现最长子串的下标, 相对于str1, 也可相对于str2, 不过是判断行和列的不同, 相对于str1, 找ii
     *5.利用库函数strncpy求出最大子串内容str3
     */
    ...
    for (int i = 0; i < len1; i++)
    {
        for (int j = 0; j < len2; j++)
        {
            ...
            if (arry[i][j] > maxlen)
            {
                maxlen = arry[i][j]; maxend = i;
            }
        }
    }

    strncpy(str3, str1 + maxend - maxlen + 1, maxlen);
    str3[maxlen+1] = '\0';

    return maxlen;
}

// 将域名转化为IP地址，成功返回true，失败返回false，结果集存放在容器中
bool hostname_to_IP(const char *urlbuf, std::vector<std::string>& v_ip)
{
    struct hostent *host; // 域名结构体
    std::string s;

    host = gethostbyname(urlbuf);
    ...
    {
        s = std::string(inet_ntoa(*((struct in_addr *)host->h_addr_list[i])));
        v_ip.push_back(s);
    }

    if (v_ip.empty()) return false;
...
bool runCmd(const std::string& cmd, std::string& context)
{
    ...
    char buf[2048];
    context = "";
    while(fgets(buf, 2048, fd) != nullptr)
    {
        context.append(buf);
    }
